 ...
 * @class 分页控件, 使用原生的JavaScript代码编写. 重写onclick方法, 获取翻页事件, 可用来向服务器端发起AJAX请求.
 * 
 * @param {String}
 *            id: HTML节点的id属性值, 控件将显示在该节点中.
 * @returns {PagerView}: 返回分页控件实例.
 * 
 * @example ### HTML: &lt;div id="pager"&gt;&lt;/div&gt;
 * 
 * ### JavaScript: var pager = new PagerView('pager'); pager.index = 3; //
 * 当前是第3页 pager.size = 16; // 每页显示16条记录 pager.itemCount = 100; // 一共有100条记录
 * 
 * pager.onclick = function(index){ alert('click on page: ' + index); // display
 * data... };
 * 
 * pager.render();
 * 
...
var PagerView = function(id) {
	...
	this._calculate = function() {
		...
		self._end = Math
				.min(self._pageCount, self._start + self.maxButtons - 1);
	...
	this.render = function() {
		...
		if (self._pageCount > 1) {
			if (self.index != self._pageCount) {
				str += '<a href="javascript://' + (self.index + 1)
						+ '"><span>&gt;&gt;</span></a>';
				str += '<a href="javascript://' + self._pageCount
						+ '"><span>&gt;|</span></a>';
			} else {
				str += '<span>&gt;&gt;</span>';
				str += '<span>&gt;|</span>';
