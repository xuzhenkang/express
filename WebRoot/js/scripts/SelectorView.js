 ...
 * @class 用于显示两栏(备选框, 已选框)选择器控件的JavaScript控件.
 * 
 * @param {String}
 ...
 * @example
 * 
 * #HTML代码: &lt;div id="sel_div"&gt;&lt;/div&gt;
 * 
 * #JavaScript代码: var sel = new SelectorView('sel_div'); sel.src.header = { id :
 * 'Id', name : 'Name', text : 'Text' }; sel.dst.header = { id : 'Id', name :
 * 'Name', }; sel.src.dataKey = 'id'; sel.dst.dataKey = 'id'; sel.src.title =
 * '可选'; sel.dst.title = '已选';
 * 
 * sel.src.add({id: 1, name: 'Tom', text: 'Tomcat'}); sel.src.add({id: 2, name:
 * 'Jerry', text: 'Jerrimy'});
 * 
 * sel.render();
 */
function SelectorView(id) {
	...
	this._init = function() {
		...
		str += '<input type="button" value="选择" onclick="document.getElementById(\''
				+ this.id + '\').view.select()" />';
		str += "</td>\n";
		str += '<td valign="top" class="dst">';
		str += '<div id="' + dst_id + '" class="dst_div"></div>';
		str += '<input type="button" value="取消选择" onclick="document.getElementById(\''
				+ this.id + '\').view.unselect()" />';
		str += "</td>\n";
		str += "</tr>\n";
		str += "</table>\n";
