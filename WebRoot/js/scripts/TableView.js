 ...
 * @class 用于显示数据表格的JavaScript控件. 集成的分页控件, 可对表格中的数据集进行客户端分页.
 * 
 * @param {String}
 *            id: HTML节点的id, 控件将显示在该节点中.
 * @returns {TableView}: 返回分页控件实例.
 * @requires jQuery {@link PagerView}
 * 
 * @example ### HTML代码: &lt;div id="my_div"&gt;&lt;/div&gt;
 * 
 * ### JavaScript代码: var table = new TableView('my_div'); table.dataKey = 'id';
 * table.header = { 'id' : 'Id', 'name' : 'Name', };
 * 
 * table.add({id:1, name:'Tom'}); table.render();
 */
var TableView = function(id) {
	...
	/**
	 * @class 用于确定要显示哪些内部控件, 控件对应的属性为Boolean类型, 取值为true时显示.
	 */
	...
	/**
	 * 添加记录列表, 如果控件已经被渲染, 会导致一次刷新. 用本方法替代连续多次{@link TableView#add()}, 以提高性能.
	 * 
	 * @param {Array[Object]}
	 *            rows: 记录对象的数组.
	 */
	...
	/**
	 * 删除一个记录对象, 如果控件已经被渲染, 会导致一次刷新. 可以在调用本方法前, 调用{@link TableView#get()}方法通过id获取要删除的记录对象.
	 * 
	 * @param {Object}
	 *            row: 记录对象.
	 */
	...
	/**
	 * 删除记录对象列表, 如果控件已经被渲染, 会导致一次刷新. 用本方法替代连续多次{@link TableView#del()}, 以提高性能.
	 * 
	 * @param {Array[Object]}
	 *            rows: 记录对象的数组.
	 */
	...
	this._update_meta = function() {
		...
		var marked_count = 0;
		marked_count = $(self.container).find(
				'.datagrid td.marker input[value!=""]:checked').length;
		$(self.container).find('.datagrid_meta span.marked_count').html(
				marked_count);
		$(self.container).find('.datagrid_meta span.row_count').html(
				self.rowCount());
	}
	...
	this._after_render = function() {
		...
		$(self.container).find('.datagrid_meta .title').css('display',
				self.display.title ? '' : 'none');
		$(self.container).find('.datagrid_meta .count').css('display',
				self.display.count ? '' : 'none');
		$(self.container).find('.datagrid_meta .filter').css('display',
				self.display.filter ? '' : 'none');
		$(self.container).find('#' + self.pager.id).css('display',
				self.display.pager ? '' : 'none');
		$(self.container)
				.find(
						'.datagrid_div .datagrid th.marker,.datagrid_div .datagrid td.marker')
	...
	this._render_framework = function() {
		...
		str += '<input type="text" value="' + this._filter_text + '"'
				+ ' onkeyup="document.getElementById(\'' + this.id
				+ '\').view.filter(this.value)" />';
		str += '</span>\n';
		str += '</div>\n';
	...
	this.render = function() {
		...
		str += '<input type="checkbox" value="" onclick="document.getElementById(\''
				+ this.id + '\').view._toggleSelect()" />';
		str += '</th>\n';
	...
	this.getSelected = function() {
		var items = [];
		$(self.container).find('.datagrid td.marker input[value!=""]:checked')
				.each(function(i, cb) {
					if (cb.checked) {
